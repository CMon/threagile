...
func (adoc adocReport) writeDefaultTheme() error {
...
func (adoc adocReport) WriteReport(model *types.Model,
	...
	modelHash string,
	introTextRAA string,
	customRiskRules types.RiskRules) error {
	...
	// add threagile header and title page with logo

	// Improvements TODO:
	// * add documentation on how to create (with asciidoctor docker call, and custom theming)

	// Final test todo:
	// * links from the Impact Analysis* chapters
	// * links from the Assignment by functions
	// * links from the stride
	return nil
}
...
`
	...
	if len(adoc.model.ManagementSummaryComment) > 0 {
		writeLine(ms, "\n\n\n"+adoc.model.ManagementSummaryComment)
	}
...
func (adoc adocReport) targetDescription(f *os.File, baseFolder string) {
	...
	writeLine(f, adoc.model.BusinessOverview.Description)
	addCustomImages(f, adoc.model.BusinessOverview.Images, baseFolder)

	writeLine(f, "\n\n")
	writeLine(f, "== Technical Overview")
	writeLine(f, adoc.model.TechnicalOverview.Description)
...
func (adoc adocReport) raa(f *os.File, introTextRAA string) {
	...
	writeLine(f, ":fn-risk-findings: footnote:riskfinding[Risk finding paragraphs are clickable and link to the corresponding chapter.]")
	writeLine(f, "")
	writeLine(f, introTextRAA+"{fn-risk-findings}")
	writeLine(f, "")
...
func (adoc adocReport) riskTrackingStatus(f *os.File, risk *types.Risk) {
	...
	if tracking.Status != types.Unchecked {
		...
		writeLine(f, `
[cols="a,c,c,c",frame=none,grid=none]
|===
...
func (adoc adocReport) riskCategories(f *os.File) {
	...
	for _, category := range adoc.model.SortedRiskCategories() {
		...
		writeLine(f, category.Description)
		writeLine(f, "*Impact*::")
		writeLine(f, category.Impact)
		writeLine(f, "*Detection Logic*::")
		writeLine(f, category.DetectionLogic)
		writeLine(f, "*Risk Rating*::")
		writeLine(f, category.RiskAssessment)
